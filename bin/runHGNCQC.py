...
def process():
    global rptOne, rptTwo, hgncIdOnlyCount, ncbiIdOnlyCount
	...
	if egId == '' and mgiIDs == '':
	    hgncIdOnlyCount += 1
	    continue
	# if egId, but no mgiId, skip until we determine how to 
	# cluster human only
	elif egId and mgiIDs == '':
	    ncbiIdOnlyCount += 1
	    continue
	...
	for id in mgiIDList:
	    ...
	    if id not in mouseMarkerIdList:
		error = 1
...
print 'lines Only Human NCBI ID: %s' % ncbiIdOnlyCount
